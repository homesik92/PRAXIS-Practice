...
| ~~B-7~~ | 2026-08-16 | ~~Decide the 5165 on-screen calculator fork.~~ | **Closed** — D-14: build a scientific (non-graphing) calculator. |
| B-9 | 2026-08-16 | Build the 5165 scientific calculator per D-14: arithmetic, log, trig (and inverse trig) functions, keyboard-operable, screen-reader-usable output, no color-only state signaling (§1.3 rules apply). Explicitly no graphing. Size as its own coding-plan task alongside the static reference panel (B-5), not folded into it. | Open — for the coding plan |
