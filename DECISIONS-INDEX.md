...
| Relationship to native iOS wrapper apps | **D-30** (each subject its own `PRAXIS-iOS-<subject>` repo; flag PRs touching shared engine files) → **D-37** (downstream reversed to one repo, one Xcode target per subject — the flagging obligation is unchanged, the destination is now a single repo) |
