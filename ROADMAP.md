...
- ☐ **11.2 Subject picker and the entitlement boundary.**
  ...
  *Progress:* Phases 0, A, B, C and D merged, with the lock-signal carry-over fix (N-21) landed
  ahead of C. Next: E (wiring — lock badges, purchase sheet, the `praxisapp://local/unlock`
  route, reload on purchase), which is when the app first shows locked content.
