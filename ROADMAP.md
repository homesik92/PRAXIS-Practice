...
- ☐ **11.2 Subject picker and the entitlement boundary.**
  ...
  `ContentView`'s hardcoded single subject. **Design settled 2026-09-16, D-46** (amends
  D-42, resolving [#131](https://github.com/homesik92/PRAXIS-Practice/issues/131)): Study
  a topic stays free unlimited; Category test is free once per topic; everything else
  (full test, Practice a topic, Review a topic, repeat Category tests) needs the subject's
  one-time unlock. A data-driven "locked" signal reaches the web layer — purchase state
  itself never does — checked per-topic against Category test attempt history rather than
  a static per-subject flag. StoreKit 2 itself is testable locally against an Xcode
  `.storekit` configuration file, with no App Store Connect record — that is architecture
