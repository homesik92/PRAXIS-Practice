...
5165 and 5652 are now fully authored. The other two tests (5101, 5485) remain
at 0 authored questions — **not scheduled until after Phase 8.3 (v1 accepted),
per D-24, though the session owner is choosing to build them out now rather
than wait**, ahead of turning each into its own iOS wrapper app per the
downstream-native-apps plan (CLAUDE.md). Resumes at the same 3×-depth,
high-effort, independently-verified standard 5165 and 5652 used: 5101
(120×3=360), 5485 (125×3=375) — 735 questions total across the two, each batch
session labeled by test code per SKILL.md's triage conventions. Feeds Phase
9.2's manifest re-enable and Phase 10's final acceptance.
...
- ☐ **9.1 Mockup, iterate to sign-off.** Not started.
- ☐ **9.2 Re-enable manifest + build S1.** `data/manifest.json`: flip
  5101/5485/5652 back to `enabled: true` as each has real content (Phase 7
  resumed, below) — doesn't have to wait for all three at once. `index.html`
  gains a per-subject progress indicator (attempted/not-yet-attempted at
  minimum; richer if the mockup calls for it) using `js/store.js`'s existing
  cross-attempt helpers, same data `dashboard.html` already reads.
...
reaching the same 3×-depth, answer-key-verified standard as 5165) and Phase 9.
