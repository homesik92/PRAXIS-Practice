...
how far the two overlap. They overlap heavily but are not interchangeable — see
BLUEPRINT.md's "5436 vs 5485" and D-36.

- ☑ **7.3.1** BLUEPRINT.md — 5436 entry (150 min, 135 questions, 20/50/35/30 across four
  categories) plus the full gap analysis against 5485; F-1's authoring-load finding
  annotated, since 5436 does not add a fifth test's worth of work.
- ☑ **7.3.2** SCHEMA.md §2.12 — `derivedFrom`, the copy-not-share rationale, and array
  `testCode` for reference panels shared between tests. Logged as D-36 and indexed.
- ☑ **7.3.3** `data/tests/5436.json` — nine weight-bearing leaves summing to
  `formLength` 135; registered in the manifest as `enabled: false` until III/IV have
  content.
- ☑ **7.3.4** `tools/derive-5436.mjs` — re-runnable, idempotent, `--check` mode.
  **375 questions derived** (I-A 27, I-B 27, II-A 75, II-B 123, II-C 123). Natively
  authored questions are preserved untouched.
- ☑ **7.3.5** `validateDerivedQuestions` in `tools/verify.mjs`, wired into the run for
  every bank. **Proved it can actually fail, in both directions**, before trusting it:
  tampering with a copy is caught, and correcting an answer key in 5485 alone turns both
...
- ☑ **6.10.3 Business Education (5101) — 8 chapters, done.** I Accounting and
  ...
  Business · VII Marketing and Management · VIII Professional Business
  Education. This bank's categories are flat top-level leaves (no
  subcategories — the session owner's call during 5101's question authoring),
  so each chapter covers a broader span than 5652's or 5485's subdivided ones.
  Authored via 8 parallel per-category subagents (high effort), each producing
  the same 3-section shape as 5652's/5165's chapters; zero id collisions on
  merge into `data/teaching/5101.json` (24 sections, 123 entries).
  **Independent verification, 4 subagents (one per category pair), each
  re-deriving worked-example arithmetic/reasoning from scratch before
  comparing** — same standard as 6.10.1. Found and fixed 2 real defects: II's
  worked example claimed a "chronological" résumé while explicitly reordering
  entries by relevance ahead of recency, self-contradicting the chapter's own
  definition of that format two sections earlier — fixed by having the résumé
  genuinely be a combination résumé (skills summary + true reverse-chronological
  experience below), matching the chapter's own definitions instead of
  contradicting them; the same worked example also misclassified a competitive
  job-application cover letter as a "routine" message calling for a direct
  pattern, when the chapter's own taxonomy defines a persuasive message (a
  message asking a resistant reader to grant something) as the correct
  category — fixed to a persuasive, indirect (attention/interest-desire/action)
  structure. VII's worked example mislabeled a segmentation basis — "gym-goers
  seeking maximum protein per gram" was called a usage-occasion segment when
  the chapter's own definitions (repeated in both its concept overview and
  common-mistakes section) classify a desired product attribute as benefits
  sought — fixed the label and tightened the companion segment's
  characterization. Legally/factually sensitive content held up under scrutiny:
  VI's FCPA facilitating-payments framing (flagged by its own author as the
  trickiest part) was independently checked against the actual statutory
  carve-out and DOJ/SEC guidance and found accurate and appropriately hedged,
  with only an optional, out-of-scope FYI noted (CISG could presumptively
  apply to the scenario's cross-border sale instead of the UCC, not fixed
  since this is a U.S.-focused exam). V's cost-benefit worked example (a
  bakery-supply reorder-point and inventory-system decision) and III's
  economics worked example (a strawberry-market supply shock, solved
  algebraically from the raw schedule, not just checked against the table)
  both re-derived clean to the dollar/unit on independent recomputation. VIII's
  differentiation claim from the existing 36 quiz questions was checked and
  found to only partly hold — curriculum design/backward-design and
  formative-assessment content meaningfully overlaps existing items
  `5101-profed-020/022/026-030`, while lab management and the PD dual-track
  are genuinely new ground — noted here since teaching content overlapping its
  own quiz category's topics isn't actually a defect (the chapter is
  preparation for those very questions), just worth knowing it isn't the clean
  split the authoring agent believed it was. `node tools/verify.mjs` clean
  throughout; 324/324 tests green (unchanged — data-only). Live-tested in the
  browser: `teach.html?code=5101&category=II` and `category=VII` both render
  the corrected content with zero console errors.
...
- ◐ **8.2 First deploy — done, but short of this task's own Accepts criteria.**
  ...
  **Update 2026-08-31.** The first gap has since closed (a full timed attempt was
  live-tested start-to-score against the NAS, 2026-08-21). The rollback gap is still
  open and is now better understood: **rollback in practice is a redeploy from an
  earlier commit, not a folder rename**, since `main` is the source of truth and every
  deployed byte is reproducible from it — which is arguably a stronger way back than a
  kept folder, and worth rewriting this Accepts line around rather than continuing to
  track it as unmet. Also learned this session: **the `tar`-over-SSH pattern can add and
  overwrite but can never delete.** `dashboard.html`, removed from the repo in Phase
  6.8.2 PR A, had continued to serve from the NAS ever since (HTTP 200 there, 404 on
  Pages) until it was removed by hand. Any future file deletion will persist on the NAS
  the same way unless explicitly cleaned up.
...
- ☑ **10.2 Final live acceptance.** The session owner's own full pass across
  ...
  *Accepts:* session owner explicitly confirms the complete five-subject app
  is accepted as production-ready. **Confirmed 2026-09-16** — session owner
  tested all five subjects on the NAS deployment; passed, nothing to fix.
  **Phase 10 is complete, closing the web app** (D-44) — Phase 11 (iOS apps
  to production-ready) is the only phase left before App Store work.
...
- ☑ **11.1 Native shell completeness.** The backup/restore export button's silent no-op
  ([#133](https://github.com/homesik92/PRAXIS-Practice/issues/133)) is fixed: the web
  layer's Blob download now reaches iOS's share sheet via a `WKDownloadDelegate`. Its
  sibling — external links — was fixed in **D-45**, and that delegate wiring sits in the
  same file. **Checked restore as well as export:** an export that works and an import
  that doesn't is worse than neither, because it invites trust in a backup that cannot be
  restored (restore replaces the whole store, D-26).
  *Accepts:* a backup taken in the app can be saved out, and restored back into it.
  **Confirmed 2026-09-16** — both directions live-tapped in the Simulator: export
  produces a real share sheet with the exported JSON, restore opens the native iOS
  document picker (Recents/Shared/Browse), exactly as designed.
- ☐ **11.2 Subject picker and the entitlement boundary.**
  ...
  `ContentView`'s hardcoded single subject. **Design settled 2026-09-16, D-46 + D-47**
  (both amend D-42, D-46 resolving [#131](https://github.com/homesik92/PRAXIS-Practice/issues/131)):
  Study a topic stays free unlimited; Category test is free once per topic; everything
  else (full test, Practice a topic, Review a topic, repeat Category tests) needs a
  purchase. **Pricing is per subject** (D-47): $5.99 to unlock one subject, $9.99 to
  unlock every subject in a multi-subject app at once (today, only STEM has more than
  one) — a subject is unlocked if the user owns its own product or the app's bundle.
  A data-driven "locked" signal reaches the web layer — purchase state itself never does
  — checked per-topic against Category test attempt history rather than a static
  per-subject flag. StoreKit 2 itself is testable locally against an Xcode `.storekit`
  configuration file, with no App Store Connect record — that is architecture work, not
  release work.
  *Accepts:* purchase, restore, refund and revocation all behave correctly against a local
  StoreKit configuration, and the free tier matches D-46 exactly (teaching always open,
  one Category test per topic, everything else gated).
  *Progress:* every phase (0, A–E) merged, plus the lock-signal carry-over fix (N-21) ahead of
  C. **Still open until the Gate 5 live test:** a real purchase, restore and refund pass from
  Xcode's Run action against the local StoreKit products — the app has never completed a
  purchase, and that is what flips this to ☑.
...
| --- | --- | --- |
| 2026-09-18 | Start cards overflowing on iPhone | Session owner spotted it in the Simulator: on a subject's test menu, "Practice a topic" and "Study a topic" rendered their descriptions far larger than "Category test" beside them, and their dropdowns and buttons ran off the right edge. Two iPhone-only WebKit behaviours, one feeding the other: a `<select>` is as wide as its longest option, and those two pickers list indented sub-topics with long labels (Category test lists only top-level topics, which is why it alone looked right); iOS WebKit lets a stretched item in a column flexbox grow to that content width, so the card's inner row went wider than the phone; and iOS **text autosizing** then inflated the overflowing blocks' font size. Fixed in `css/base.css` only: `text-size-adjust: 100%` on `html` (switches off the autosizing guesswork; a reader's own text-size setting still applies, N-19), and in the stacked phone layout the card's children are capped at its width and the select claims no width of its own. **A desktop browser at 402px laid the page out correctly throughout** — the bug reproduced only in the app's `WKWebView` and in the Simulator's mobile Safari, which is where the fix was iterated. This also affected the public website on iPhones, so it ships to Pages and the NAS too. Verified in the app: all four cards fit, one text size, every button visible; all 9 UI tests pass (they tap these buttons). |
| 2026-09-17 | 11.2 Phase E — purchase flow wired up (N-23) | The phase where the app first shows locked content. A locked control in a page (`test.html`'s "Unlock to access", `run.html`'s locked screen) is a plain link to `praxisapp://local/unlock?code=<code>`; `WebViewContainer.Coordinator` cancels that navigation and calls back, so no JavaScript bridge was added and the link is inert in a browser. The `code` is matched against the app's own subject list rather than trusted (D-46). New `SubjectWebView` owns the sheet state for both tabs, and new `PurchaseSheet` is the only place in the app that names a price — this subject, the all-subjects bundle, Restore purchases, each price read from StoreKit's `displayPrice`; it closes only once the subject it was opened for is genuinely unlocked, and offers a retry when products couldn't load. `WebViewContainer` now appends `unlocked` itself (one place decides what the web layer is told) and `updateUIView` reloads **only** when that value changed, which is what makes a purchase land without leaving the tab. The subject list badges locked subjects rather than blocking them, since D-46's free tier lives inside a subject. `Phase4Tests` now launches with a **DEBUG-only** `-uiTestForceUnlocked` argument, because the paid flows can't be bought in a test (#162) and deleting that coverage was the alternative; a new `testFreeTierIsUsableWhileLocked` deliberately runs without it and asserts the lock badge, that Full test/Practice/Review are locked, that Study a topic stays open, and that tapping a locked control really opens the sheet. **All 8 UI tests pass**, build clean. Simulator, launched without the flag: lock badges on all four subjects; Mathematics opens to locked controls with Study a topic open; tapping "Unlock to access" opens "Unlock Mathematics" with Restore purchases, and — launched outside Xcode, so with no StoreKit configuration attached — the sheet correctly shows "Prices couldn't be loaded" with a Try again, verifying the degraded path too. Then, at the session owner's request, the opening screen gained a **free-tier explainer**: an illustrated card above the subject list ("Try it before you buy it") and a "What's free" screen behind it, also linked from the purchase sheet — because four padlocks with no explanation reads as "everything costs money" when every lesson is free, and because an App Store reviewer meets the same screen. The copy names **no price** (prices belong to StoreKit, N-22), which a new UI test asserts. The card reuses `index.html`'s classroom illustration (D-34), extracted into the asset catalog with its CSS variables resolved to literal colors — the one image the site and the app both carry, and a real drift risk, now flagged in `ios/CLAUDE.md`. The session owner then asked for the **question pool** to be named: the card reads "1,550 questions across 4 subjects", the "What's free" screen lists each subject's count, and each purchase option shows its own — all read from the manifest, whose `bankSize` `tools/verify.mjs` cross-checks against the banks on every CI run. Two independent review rounds (code, and a dedicated paywall-bypass hunt) found no way to reach a paid mode without paying, and ten real defects, all fixed here: a purchase reloaded the tab's *entry* page rather than the page the buyer was on (which would have lost a Category test's answers mid-run); a partial product load left a subject unbuyable with no explanation; the sheet's actions could race; a restore that didn't cover this subject told the buyer to try Restore; VoiceOver got no signal while the sheet worked; the "Unlock to access" link was `aria-disabled` despite being the only way to buy; a hand-built URL with a leaf category could mint extra free trials (the gate now takes the subject's top-level ids); the manifest was re-parsed on every entitlement change; and stale comments in four files. The bypass hunt also found the free tier could be farmed through **Clear performance data** or an old backup restore — session owner chose to keep the trial record through both (`keepUsedCategoryTrials`). Node gates: `test-store` 79/79, `test-entitlement` 24/24. **9 UI tests pass**; both explainer screens confirmed in the Simulator. **Not verified: any actual purchase, restore or refund** — those need Xcode's Run action, and that is the Gate 5 live test that closes 11.2. |
| 2026-09-17 | 11.2 Phase D — StoreKit 2 entitlement layer (N-22) | The purchase layer, with nothing yet reading it — session owner's call, so the app never ships an interval where content is locked but unbuyable; it still passes `unlocked=1` everywhere and every subject opens fully unlocked. New `EntitlementStore` (`@MainActor ObservableObject`) is the only type touching StoreKit: `Transaction.updates` listener started at launch and kept for the app's lifetime, `Transaction.currentEntitlements` read at launch and after every change, unverified transactions never treated as ownership, transactions finished on delivery, `AppStore.sync()` only behind an explicit Restore, and `isUnlocked(subjectCode)` = owns that subject's product **or** the app's bundle (D-47). New `ProductCatalog` holds the subject↔product mapping natively (never in the web-shared manifest, D-46) and derives the bundle id from the subject count, so a single-subject app has no bundle tier. **No price is written in Swift** — prices come from StoreKit's `displayPrice`, with D-47's $5.99/$9.99 living in the new `ios/Configuration.storekit` (5 STEM products, local, no App Store Connect record). Product ids are placeholders pending #135. **Every StoreKit API was checked against this machine's iOS 27 SDK interface file, not recalled** (#136 asked for exactly this): all are iOS 15+, under the 16.4 floor, and `Transaction.currentEntitlement(for:)` is deprecated as of 18.4 so the `currentEntitlements` sequence is used. Also added the project's **first committed shared scheme** (`schemes:` in `project.yml`), which attaches the StoreKit configuration to the Run action and gives `xcodebuild test` something to name from a fresh clone. Verified: build succeeds; the `.storekit` file is absent from the built bundle (`buildPhase: none`); all 7 UI tests pass against the new shared scheme, so starting the store at launch regresses nothing. **Not verified by this session:** the `.storekit` file's format and the scheme's relative path to it — both need Xcode, which is the session owner's step, and a purchase/refund/restore pass against the local products is the Gate 5 live test. Xcode's Test action doesn't take the StoreKit configuration from `project.yml` (xcodegen writes it to Run only), so automated purchase tests need that wired first (#162 — the dead `test:` key was removed). Code review then fixed a real hazard: the bundle product id had been derived from the count of currently-enabled subjects, which would silently strip a paying buyer's bundle entitlement if a track ever dropped to one enabled subject — it is now a build-time constant. Also: `productLoadFailed` now trips on *any* missing product rather than only on all of them; the `Praxis` trademark was taken out of the placeholder in-app-purchase copy (D-40/LEGAL.md); and `ios/CLAUDE.md`'s list of `resourceDirectory` call sites was missing the new one. **Session owner confirmed in Xcode (2026-09-17) that `Configuration.storekit` loads in the StoreKit editor with all five products and their prices** — the format risk this session flagged as unverified is closed, and an independent read of Xcode 27's own StoreKit plugin metadata agreed the schema is right. A second review round (StoreKit/concurrency) found four more real defects, all fixed here: overlapping entitlement reads could publish a stale snapshot and drop a just-arrived purchase until relaunch (reads are now serialized, and a cancelled read publishes nothing); `restore()` reported success when the buyer owned nothing (now a distinct `.nothingToRestore`, the usual App Store review rejection); a product fetch that failed at launch could never be retried for the rest of the run (`retryLoadingProducts()` added); and `productLoadFailed` couldn't see a failed manifest. `ProductCatalog` now lists which tracks have a bundle rather than claiming a nil nobody could set, so 11.3's single-subject apps don't request a product that doesn't exist. The scheme's `../../Configuration.storekit` path turned out to be **correct** (checked against XcodeGen's documented prefix and three real Xcode-written schemes) — the scheme editor read "None" only because the project was regenerated while Xcode held it open; but setting it in Xcode did expose real scheme churn between Xcode and xcodegen (#164), now documented in `ios/CLAUDE.md`. Also corrected `verify.yml`'s comment, which justified its target-based build by the absence of a shared scheme this PR adds (#162), and trimmed the placeholder purchase copy to App Store Connect's 30/45-character limits. |
| 2026-09-16 | 11.2 Phase C — native subject picker | Both tabs now open on `SubjectPickerView`, a native list of every enabled subject in `data/manifest.json` whose `track` is `stem` (new `ManifestLoader.swift`; the constant `ManifestLoader.appTrack` stands in for 11.3's undecided multi-app structure). Practice pushes the subject's `test.html`; Study pushes a per-subject topic list (`CategoryListView`, which replaces `StudyPickerView`, with `CategoryLoader` now finding the bank by the manifest's own `file` path instead of a hardcoded `5165.json`). No lock badge yet — the app has no purchase state until Phase D, so every subject opens with `unlocked=1`. **`Phase4Tests.swift` turned out to be broken on `main` already** — a baseline run of the untouched `main` in a separate worktree failed the same 6 of 7 tests — so fixing it was in scope per the plan's "full UI-test run green before merging": a hierarchy dump showed WebKit now reports a radio option's frame starting at the top of the question stem (so a centre tap lands on the stem and is refused — now tapped at the radio's own corner), the backup `<summary>` is now exposed as a Button, the closed confirm dialog's "Submit test" button is always in the tree (review screen's taken first), and a leftover in-progress attempt skips run.html's Start screen (now tapped only if shown). Each test now picks its subject by accessibility identifier first, and two checks that could never fail were made real (code review): persistence now asserts the "Review test" link that only a completed attempt renders — the score ring's "Not started" is inside an SVG exposed only as an image — and Study waits for teach.html's own Back link rather than a label the list row already carried. The subject list uses value-based navigation so the Study tab no longer parses every subject's bank up front (code review). **All 7 UI tests pass** via `xcodebuild test` (the locally-generated `PraxisMath` scheme — `ios/CLAUDE.md` now documents this). Simulator: Practice lists exactly Mathematics, General Science, Physical Science and Computer Science; Study → General Science → Chemistry opens its teaching page. `ios/CLAUDE.md` updated for the new files, the manifest-driven subject list, and the settled purchase-signal rule (#131 was still described as open). Site files untouched. |
| 2026-09-16 | 11.2 — carry the lock signal across pages (N-21) | Found while preparing Phase C: Phase A's `unlocked` signal was dropped by every link between site pages, and a page without it treats itself as the public site, so "← Back" links, `run.html`'s "Try again"/review links, `results.html`'s weak-spot links and `test.html`'s hub link all led past the paywall (nothing is for sale yet, so no one was affected). Session owner chose to fix it in its own PR before Phase C. `js/entitlement.js` gains `withEntitlementParam` (every inter-page link now goes through it) and `isRunLocked` (`run.html` re-checks entitlement on arrival and shows an "Unlock to access" screen instead of the run); `test.html` hides "← Choose a different test" inside the app; `WebViewContainer` cancels any navigation to `index.html`; the app now passes `unlocked=1` so the web layer knows it's in the app while everything stays open. `tools/test-entitlement.mjs` 21/21 (10 new), all other suites and `tools/verify.mjs` green, iOS build succeeds. Live-verified in the browser: public-site links unchanged and hub link visible; with `unlocked=0` the hub link is hidden, the free links carry the signal, `run.html` shows the unlock screen for full test/due review/an already-tried topic but opens an untried topic's Category test, and back links carry the signal; with `unlocked=1` everything runs. Simulator: the Practice tab loads with the hub link gone and every control open. Code review (`/code-review` unavailable in-session, so its method was run by hand with two independent reviewers) found no bypass and four things fixed in the same PR: changing the Practice-a-topic dropdown re-pointed its locked button at the run (lock now re-applied on that change too); Back links on error pages (`results.html` with an unknown attempt, `run.html`/`teach.html` with no code) still pointed at the now-blocked hub, so they're hidden in the app; "D-19" had been cited for the purchase boundary, which in this repo's log is the question-order decision — corrected to D-46 / iOS D-19 (including two Phase A comments); and a Swift comment overstated which pages hide their hub links. Both behavior fixes re-verified in the browser. |
| 2026-09-16 | 11.2 Phase B — `track` field on the manifest (#121) | Second implementation PR of the approved 11.2 plan. `data/manifest.json` gains `track` per D-41/D-43 (5165/5436/5485/5652 → `stem`, 5581 → `humanities`, 5101 → `admin`) — issue #121/#136's premise that this field already existed was false, checked directly against the file this session. `tools/verify.mjs`'s `validateManifest` warns (not errors) when a subject has no track yet, since a new subject can land before its app assignment is decided, but errors on a present-and-invalid value. `js/schema.js`'s `loadManifest` now carries `track` through — kept as its own explicit copy, not folded into `DISPLAY_FIELDS`, since unlike those four fields it isn't cross-checked against the bank file. `SCHEMA.md` §2.2's manifest example was stale independent of this change (missing the four display fields entirely) — corrected in passing since this PR was already touching that section. Also fixed a real gap found while reviewing CI config: Phase A's `tools/test-entitlement.mjs` was never wired into `.github/workflows/verify.yml`, so it silently never ran in CI despite passing locally — added here. `node tools/verify.mjs` (0 errors/0 warnings against the real, now-fully-tracked manifest), `tools/test-verify.mjs` (71/71), `tools/test-schema.mjs` (85/85) all green. Live-verified `index.html` renders all 5 enabled subjects unchanged. Gate 4 (schema-tier change) — held for explicit go-ahead before merge, not merge-on-green. Phase C (native subject picker) is next, and now unblocked. |
| 2026-09-16 | 11.2 Phase A — free-trial state + locked-control rendering in test.html | First implementation PR of the approved 11.2 plan (`~/.claude/plans/cuddly-twirling-toucan.md`). `js/store.js` gains `categoryTrialsUsed` (`recordCategoryTrialUsed`/`hasUsedCategoryTrial`, extending `clearTestData` to reset it) — deliberately separate from `attempts`, since a Category test run isn't a recorded attempt and reusing that array would corrupt the stats dashboard. New `js/entitlement.js` is the one pure module that turns the app's `unlocked` URL param plus that store state into a per-control lock decision, with the hard rule that an absent param (the plain public site, always) resolves every control to unlocked — verified both by 11 new `tools/test-entitlement.mjs` cases and a live check serving the site locally with no `?unlocked=` param. `run.html`'s `runCategoryDrill` now records a topic's trial as used when a *timed* drill completes (best-effort, unconditional — harmless where nothing reads it). `test.html` renders Full test/Practice a topic/Review a topic/an already-tried Category test as visibly locked, linking to `praxisapp://local/unlock?code=<code>` — reusing the existing custom-scheme mechanism `WebViewContainer`'s `Coordinator` already handles for external links (D-45) and downloads (#133), not a new bridge. Live-verified in the browser (the dev-preview pane's own HTTP cache went stale mid-session and needed an origin swap to bypass — a tooling quirk, not a code defect, confirmed three independent ways including a live cache-busted import matching the Node test results exactly): locked rendering, the `praxisapp://` hrefs, live re-lock on switching the Category-test topic dropdown, and byte-for-byte unaffected rendering with no `unlocked` param. `node tools/verify.mjs`, `tools/test-store.mjs` (75/75), `tools/test-entitlement.mjs` (11/11) all green. Phase B (the manifest's `track` field) is next, independent of this PR. |
| 2026-09-16 | D-47 — pricing changed to per-subject, plus a full 11.2 implementation plan | Continuation of the session below: with D-46 unblocking 11.2's design, the next step was planning 11.2's actual build (two Explore agents plus a Plan agent, researching `test.html`'s Start hub, `js/store.js`, `data/manifest.json`, and the native `WebViewContainer`/`StudyPickerView`/`CategoryPicker` precedent in full). Two real findings surfaced: `run.html`'s drill runner never persists anything today (drills are explicitly "not recorded"), so D-46's free-trial gate needs genuinely new store state, not a reuse of D-26's `attempts` array as D-46's own text assumed; and the manifest has no `track` field yet despite #121/#136 assuming it does. Mid-planning, the session owner asked whether this work touches the NAS-deployed public site at all — clarified that the entitlement concept itself is app-only (confirmed by #131's own "public website is unaffected by construction" property), but a few shared-file changes are unavoidable per D-18/D-38's "one copy of each file" rule; session owner chose the richer option (test.html visibly renders locked state via a native-supplied boolean) over a purely-native-interception alternative. Separately, the Plan agent's research found D-42's original one-unlock-per-app model conflicted with the session owner's initial "pay for the subject" framing; **resolved as D-47**, amending D-42 a second time: $5.99 per individual subject, $9.99 to unlock every subject in a multi-subject app at once (only STEM has more than one today), single-subject apps (Humanities, Administrative) sell their one subject at $5.99 with no bundle tier. Full 5-phase implementation plan approved (saved at `~/.claude/plans/cuddly-twirling-toucan.md`): Phase 0 (this entry, D-47 logged), Phase A (web: free-trial store state + `test.html` locked rendering, independent), Phase B (`track` field on the manifest, Gate 4), Phase C (native subject picker, depends on B), Phase D (StoreKit 2, parallel with C), Phase E (wiring: purchase sheet + `praxisapp://local/unlock` route, depends on C+D). `DECISIONS-INDEX.md`'s pricing row and this file's 11.2 entry updated to match. Docs only in this entry — Phases A–E land as their own separate PRs. |
| 2026-09-16 | D-46 — free tier redefined, unblocking 11.2 | Continuation of the session below: with 11.1 closed, the next open item was 11.2, blocked on [#131](https://github.com/homesik92/PRAXIS-Practice/issues/131)'s unresolved design question (D-42's free tier named a "short diagnostic" the engine doesn't have, left "the complete bank" undefined, and assumed a whole-subject lock/unlock boundary that can't express D-42's per-feature gating at all). Session owner proposed a concrete alternative: one free Category test per topic as the diagnostic, Study a topic staying free as before, everything else (full test, Practice a topic, Review a topic, repeat Category tests) behind the subject's one-time unlock. Three follow-up clarifications resolved via AskUserQuestion before logging: topic granularity is the grouped picker bucket (not the finer leaf-category tree), teaching stays free, and Practice a topic is always paid (not folded into the same free try as Category test). Logged as **D-46**, amending D-42; resolves #131 without reopening its "purchase state never enters the web layer" architectural answer — only *what* gets checked changes (a per-topic usage count against Category test attempt history, D-26, instead of a static per-subject flag). #131 closed. `DECISIONS-INDEX.md`'s pricing row and this file's 11.2 entry updated to match. Docs only — 11.2's actual implementation (native picker, StoreKit 2, per-topic locked-state rendering in the web layer) is deliberately deferred to its own planning pass, per #136's own note that it's "the largest single piece of iOS work scheduled." |
| 2026-09-16 | 11.1 complete — export and restore both live-verified ([PR #151](https://github.com/homesik92/PRAXIS-Practice/pull/151)) | Continuation of the session below: once the Simulator's unresponsive-input issue was root-caused (a stale macOS Accessibility/TCC grant for the Claude desktop app, not a device or coordinate problem — fixed by toggling the grant off/on plus a full Mac restart; findings recorded in the session owner's global Claude Code config for reuse across projects), both halves of #133 were tapped through live on an iPhone 17 simulator: **export** — "Download progress" produced a real `UIActivityViewController` share sheet showing the exported JSON (e.g. `praxis-practice-progress-2026-09-...json`) with Copy/Save to Files/More; **restore** — "Upload progress" opened the native iOS document picker (Recents/Shared/Browse tabs), confirming the platform assumption in the code review below ("WKWebView has handled `<input type="file">` natively since iOS 9") held. A few early taps at "Upload progress" appeared to do nothing, initially read as a possible restore-side bug — turned out to be stale scroll-position coordinates missing the button entirely, not an app defect; resolved by re-locating the button in a fresh screenshot each time rather than reusing coordinates across scroll states. `Phase4Tests.swift`'s `testBackupExportTap`/`testBackupRestoreButtonReachable` doc comments updated to drop the "unverified this session" caveats now that both have been seen to pass. 11.1 flipped to ☑ — first item in Phase 11 to close. Phase 11 overview row stays ◐ (11.2 and 11.3 still open). |
| 2026-09-16 | 11.1 (export half) — `WKDownloadDelegate` implemented, not yet live-tested | `WebViewContainer.swift`'s `Coordinator` now implements `WKDownloadDelegate` (#133): `navigationAction.shouldPerformDownload` (verified against Apple's official docs, iOS 14.5+) routes test.html's Blob-URL `<a download>` export to a `WKDownload`, written to a temp file, handed to a `UIActivityViewController` share sheet — with the iPad popover configuration `UIActivityViewController`'s own docs say is required, or it crashes there. High-effort code review (3 parallel finder angles + verification) caught and fixed 5 real issues before push: a race where two overlapping downloads could cross-associate files (fixed with a dictionary keyed by the `WKDownload` instance instead of one shared property), a silent no-op with no cleanup when no presenter view controller is found mid-transition (fixed by widening the scene-activation filter), no failure-path handling at all (added, clearing the pending entry and any partial file), the exported temp file never being deleted after the share sheet closes (added a completion handler that does), and a hard `XCTFail` in `Phase4Tests.swift` asserting share-sheet chrome (`Cancel` button) that had never actually been seen to pass — reverted to the file's own established "observe, don't assume" pattern rather than risk a false-red regression. **Not done:** restore's `<input type="file">` is expected to already work (iOS `WKWebView` has handled this natively since iOS 9, unlike Android's WebView) but is explicitly unverified this session, and the export fix itself has not been live-tapped either — this session's Simulator control tooling had an unresponsive input pipeline (screenshots worked, taps didn't) that a device reboot, a full Simulator.app restart, and the session owner restarting the whole Mac all failed to fix. 11.1's acceptance criteria ("a backup taken in the app can be saved out, and restored back into it") is therefore not yet met; #133 stays open with this progress noted, pending a live tap-through once the tooling issue clears. |
...
| 2026-09-15 | N-17 pass over 5652's teaching chapters (#128, third of four) | `data/teaching/5652.json` (Computer Science), 30 sections / 136 entries across all ten categories, verified independently. This file is mostly *executable* content — pseudocode traces, a cipher, a simulation — so rather than reading the traces, every one was run: selection sort on [29, 10, 14, 37, 13] (10 comparisons, 3 swaps, final [10, 13, 14, 29, 37]); the guarded while loop with the compound AND condition (10 passes, output 5 / 5 / 30); the scope trace (216 returned, caller's `price` unchanged at 200); the gradebook decomposition (90 / 80 / 90 → 88 → "B"); 156 → 10011100 and back, plus the 1011100 → 0101 1100 = 5C grouping counterexample (both = 92) and 2A₁₆ = 42; the Caesar cipher round-tripping MEET AT NOON ⇄ TLLA HA UVVU through both wrap-around cases; the population simulation (2720.97792, sum 9733.20192, average 2433.30048, 36.05% vs the naive 32%); and the filter-before-aggregate example (73 vs 80.67). **Every computation in the file was correct.** Only **two defects**, the lowest rate of any pass so far (1.5%), and both are miscounts in asides rather than errors in the traces they accompany: the III-A trace's counterfactual said that a `<` instead of `<=` would have stopped the loop "with total still 12" when the correct value is 20 (pass 8 had already added 8 — the exit point named, the top of pass 9, was right), and the V-B IPv6 example said that `2001:0db8:85a3::8a2e:0370:7334` shows "only seven" of its eight groups when six are visible and `::` elides two. Also swept: no self-referential exam statistics, and no markdown in any of the 127 `text` entries (the 6 `code` and 3 `mathml` entries are correctly typed). **Six passes, six sessions, every defect still in prose rather than in a definition or a computation.** Remaining under #128: 5101 (123 entries) — the last one. |
| 2026-09-15 | N-17 pass over 5485's teaching chapters (#128, second of four) | `data/teaching/5485.json` (Physical Science), 27 sections / 140 entries across all nine categories, verified independently — no session of ours drafted it. Every worked example re-derived: Tc-99m 20.0 mg over 5 half-lives → 0.625 mg (3.1%); MgCl₂ molar mass 95.21 g/mol → 7.14 g for 0.150 mol Cl⁻; the mixed circuit (2 Ω parallel, 4 Ω total, 6 A, 12 V, 2 A / 4 A, branch currents summing back to the total); calorimetry to 30.04 °C with both back-substitutions agreeing (12,593 J vs 12,590 J); the friction problem (117.6 N → 29.4 N → 20.6 N → 1.717 m/s², 6.87 m/s, 13.73 m, confirmed by the average-velocity form); the limiting-reagent problem (O₂ limiting, 85.0 g Al₂O₃, 9.00 g Al left, mass check 45.0 + 40.0 = 85.0); and the pH problem (1.70 → 2.40, matching log 5 = 0.699). **All seven worked examples were arithmetically correct — every defect was in prose.** Six found, and for the first time in five passes two of them were substantive rather than incidental asides. (1) The Lewis-structure procedure said to count valence electrons by adding "the group number of every atom", which is broken under the 1–18 numbering the same chapter uses throughout (carbon would contribute 14) — repeated in the common-mistakes entry. (2) The MgCl₂ example called a 1.85 electronegativity difference "well above the roughly 1.7–2.0 cutoff" when 1.85 is *inside* that band — and the chapter's own common-mistakes entry cites HF at 1.78 as "near the line" and polar covalent. (3) The experimental-design worked example claimed the observed temperature gap "could be entirely explained by the black can's shorter distance and the silver can's smaller volume", contradicting its own preceding sentence: the smaller volume should have made the silver can warm *more*, so it cuts against the observed result instead of explaining it — one rival explanation, not two. (4) A correlation-versus-causation list promised "three reasons besides one causing the other" and then listed four items, the first of which was one causing the other. (5) The pH shortcut claimed dilution raises pH by log(factor) "regardless of what the starting concentration happened to be", contradicting the water-autoionization caveat two entries earlier — the rewritten version notes that dilution can approach pH 7 but never reach or pass it. Also checked: 5485 makes no self-referential claims about its own exam weighting (5165 made two), all 140 entries are `format: text` with no markdown (N-18). **Five passes, five sessions; every defect still lands in prose rather than in a definition or a computation.** Remaining under #128: 5652 (136 entries), 5101 (123). |
| 2026-09-15 | N-17 pass over 5165's teaching chapters (#128, first of four) | First of the four pre-N-17 teaching files: `data/teaching/5165.json`, 18 sections / 79 entries across all six Mathematics categories, verified independently (this session drafted none of it). Every worked example was re-derived rather than read: 0.454545… → 45/99 → 5/11; x² − 2x − 15 factoring to roots 5 and −3; the inverse of (2x+1)/(x−3) solved again to (3x+1)/(x−2); f(x)=x³−4x giving slope 8 and tangent y = 8x − 16 at (2,0); the 7-24-25 ladder; 18/30 = 0.6. All correct, as were the similarity scale-factor ratios (4:9 area, 8:27 volume) and the mutually-exclusive-versus-independent argument. Two internal claims were checked against the repo rather than assumed — "roughly a quarter of all 5165 questions" matches BLUEPRINT.md F-4's ~25%, and "13 of 66, about 20%" matches the bank's II-A weight exactly. **Two defects, both in explanatory asides, neither in a definition.** II-A explained the inverse's domain restriction with the wrong reason (claiming 2 "was in f's denominator after the swap"; it is excluded because y = 2 is f's horizontal asymptote and so missing from f's range) — and the sentence's *next* clause states the correct general rule, which is exactly what made it easy to read past. II-B asserted that average and instantaneous rates of change "only coincide when the function is linear over that interval", which is false: the average rate of x³ from −1 to 2 is 3 and f′(−1) is also 3; the counterexample now appears in the entry. That makes it four passes, four different sessions, and **every defect found so far has been in a connecting clause or an aside**. Remaining under #128: 5485, 5652, 5101 — 399 entries, one file per session. |
| 2026-09-15 | N-17 verification pass over 02b and 02c teaching chapters | Claim-extraction pass over all four chapters drafted in 02b and 02c — III-A Cells and Genetics, III-B Evolution and Ecology, IV-A Astronomy, IV-B Earth Science: 12 sections, 64 entries. Claims were pulled mechanically (every number, every always/never/only/cannot/all, every cross-reference) rather than by re-reading, and each chapter was read against its own common-mistakes entries for contradiction. **02b: substance clean** — the four conditions for selection, the unit-of-evolution distinction, the 10% rule, Punnett genotype-vs-phenotype ratios, osmosis direction, succession, and the nuance that an *energy* pyramid is always widest at the producers (true for energy, not for biomass) all hold. **One defect found, in III-A**, and in exactly the place N-17 predicts — not a definition but a connecting clause: `cells-concept-overview-2` pointed the reader at "the next section" for plants having mitochondria as well as chloroplasts, when the next entry is membrane transport and the photosynthesis/respiration entry is two later. Fixed to name the entry instead. **02c's chapters were checked the same way and nothing failed, but that pass does not count**: the session that drafted them ran it, which is the arrangement N-17 exists to forbid. Filed as its own issue so an independent session picks it up. Extraction found 25 claim-bearing sentences in IV-A and 22 in IV-B; internal consistency of the road-cut worked example (fault younger than layer 3 and older than layer 4; intrusion between layers 2 and 3) was re-derived from the scenario rather than trusted. |
| 2026-09-15 | session-02c — 5436 Earth & Space teaching chapters; `data/teaching/5436.json` complete (#106) | **IV-A Astronomy** and **IV-B Earth Science**, 6 sections and 32 entries in the standard Concept overview / Worked example / Common mistakes triple, taking the file to **27 sections / 144 entries across all 9 leaf categories** — every subject now has complete teaching content. Written from scratch: 5485 has no Earth or space science, so like 02b there was no sibling subject to derive from. Worked examples were chosen for the misconceptions they break rather than for calculation: *why eclipses do not happen monthly* (the Moon's 5° orbital tilt against a 0.5° target, and why two cycles of different lengths coincide only in eclipse seasons), and *ordering events in a road cut* from superposition, cross-cutting, inclusions and an unconformity — which also separates relative sequence from radiometric ages, a distinction the common-mistakes entry then names outright. N-18 check: zero asterisks on both rendered pages, confirmed in a browser against the local static server, not in the JSON. N-17 self-check applied mechanically to my own prose (every number, every absolute) and each chapter read against its own common-mistakes entries for contradiction — nothing failed, **but that is not the independent pass**: 02b's chapters and now 02c's both still owe one in a session that did not draft them. `verify.mjs` clean, 7 suites green. One near-miss worth recording: `.claude/launch.json` already existed with an equivalent static-server config, and a plan step to "add" it silently overwrote the port before `git status` caught it — reverted, and the existing config used instead. |
...
| 2026-09-10 | session-02b — 5436 Life Science chapters, and a rendering defect in shipped content (#106, #129) | **III-A Cells and Processes Including Genetics** and **III-B Evolution, Diversity of Life and Ecology** — 6 sections, 32 entries, written from scratch (5485 covers no life science, so unlike 02a there was no source to draw on). Worked examples chosen for the misconceptions they break: a two-black-parents/white-pup cross for genotype-vs-phenotype and per-offspring probability independence, and six seasons of pesticide resistance for the subject's largest misconception — that individuals adapt in response to need. **A presentation defect surfaced during live verification and not by reading the JSON (N-18):** teaching content is plain text, `renderContent` dispatches only on `text`/`mathml`/`code`, and 5436 carried **37 markdown emphasis spans / 80 asterisks** — 18 of which had shipped and deployed in #125. One III-B page rendered 38 literal asterisks with zero `<em>`/`<strong>`. All stripped; the ten spans where emphasis carried real meaning were rewritten to carry it in the wording. The other four teaching files are clean (every asterisk there is a multiplication operator in a formula or pseudocode). It survived #125's review because nothing rejects it — `verify.mjs` does not read prose, and `*emphasis*` looks like emphasis in the editor, the diff, and the PR view; the rendered page is the only surface where it does not, and it was the one nobody opened. **N-17's method, applied to the new prose before commit, caught two of my own** — "there is no way for it not to" about a favoured trait rising, contradicted by the genetic-drift paragraph in the very next entry, and "all life uses the same genetic code" softened to "essentially all." Both in connecting clauses, which is N-17's whole finding. Live-verified locally and then on the NAS: III-A, III-B and the III branch node all render, zero console errors, zero asterisks, and `?category=IV-A` still degrades to "No lesson has been written for 'Astronomy' yet" — so 02c ships safely later. Gate green throughout (0 errors, 68/68). **Not done, and deliberately:** per N-17 these two chapters have not had their independent verification pass; it belongs in a different session than the one that drafted them. |
| 2026-09-10 | Verification pass over 02a's teaching prose — four factual defects fixed (N-17, #127) | The first verification pass over teaching content run separately from the drafting pass, the way this project already verifies an answer key. 83 numeric, absolute and superlative claims extracted **mechanically** from 5436's 15 sections and checked in isolation rather than by re-reading — which is what kept the drafting pass's own reasoning from being re-run and re-confirmed. I-A and I-B clean; **four defects in the physical-science half.** II-A called alpha/beta decay "the one genuinely elemental change in all of chemistry and physics" two sentences before describing fission and fusion. II-A's "the spark cannot warm the tub" stated the exact misconception that II-A's own common-mistakes entry exists to correct. II-B's dilution sanity check claimed a thousandfold dilution "must still eventually produce the same mass of precipitate" — below silver chloride's solubility product nothing precipitates, and worse than the wrong fact, it taught extreme-case extrapolation as a *method* at the one point where the method breaks. II-C listed electric motors as an application of induction; motors run on force-on-a-current. **The pattern is the finding:** every one sits in a connecting clause — the "which is why," the sanity check, the summarizing aside — and none in a primary definition. The definitions were drafted deliberately and are correct throughout. Three of the four are catchable with no outside source, by reading a chapter against its own common-mistakes entries. Honest limit, recorded in N-17: same session, same author; mechanical extraction is a partial substitute for independence — it catches wrong facts and internal contradictions, not a wrong framing both passes would share. **#128** filed for the four pre-5436 teaching files, which never had this check and ship inside a paid app. |
| 2026-09-10 | D-43 — Core becomes a fourth track and its own app | Session owner's call, amending D-41. Core's audience is the one genuinely different in kind — pre-programme candidates who have not yet declared a teaching subject, assessed on basic reading, writing and mathematics rather than any subject's content. Folding it into the STEM app would sit a basic-skills mathematics test beside a mathematics-content licensure test for future maths teachers: adjacent in name, unrelated in purpose. D-41's disjoint-audience reasoning applies with more force here, not less. It also keeps the funnel legible — Core is the app a candidate meets first and the one that can credibly point at the others, which is the sequential relationship the Subject Assessments lack with each other (D-42). **Changes nothing operationally:** Core stays parked (N-16), the STEM app still ships and must be approved first, and whether to build Core at all is still deferred until there is real revenue and review data. Recorded now so the track vocabulary is complete and #121's `track` field is specified once rather than twice — it gains a fourth value, `"core"`, which no subject carries today since 5713/5723/5733 have no banks. |
| 2026-09-10 | session-02a — 5436 teaching chapters, the five derivable categories (#106) | **The sizing question #106 opens with was answered with evidence, not inference: it is a hybrid, and it is mostly authoring.** Mapping 5436's nine leaf categories against 5485's teaching layer: I-A and I-B carry identical labels and near-identical weights (9 vs 10 questions) so transfer directly; II-A, II-B and II-C have 5485 source at *different granularity* (21 sections must become 9); and **III-A, III-B, IV-A, IV-B have no source at all** — Life Science and Earth & Space Science. The question derivation already proved that boundary: I-A was 27 derived / 0 authored, while III-A/III-B/IV-A/IV-B were 0 derived and 195 authored. No derivation tooling was built — `derive-5436.mjs` has zero mentions of teaching, `derivedFrom` is question-level, and only 2 of 9 categories map cleanly, so a script would cost more than it saves. **Two findings while scoping.** `teach.html:92` renders a per-category "No lesson has been written for X yet" fallback, so staged delivery degrades honestly rather than dead-ending — which is what makes the split safe, and it was live-verified. And **N-7's landmine is real in the teaching content too, not just questions**: diffing `.authoring/teach-5485-nature.json` against the live file found `nature-worked-example-step-3` at 1333 chars versus 1352 live — a fix made in the bank and never mirrored back. Harmless today since no merge script touches teaching, but it means `.authoring/` is not a trustworthy source of truth here, so 5436's chapters were written **directly to `data/teaching/5436.json`** and committed, git being the durability mechanism rather than a gitignored staging copy. This session delivered the five categories with 5485 source: **15 sections, 80 entries, 77KB**, each as the established Concept overview / Worked example / Common mistakes triple. Session owner's call that the compressed chapters match **5436's own weighting** rather than condensing 5485's — Chemistry is 19 questions here against 5485's 41, so it is genuinely shorter and pitched broader-and-shallower, which meant real rewriting rather than excerpting. Worked examples were written fresh and pitched across life and earth science (a confounded plant-growth study, a lake restoration) rather than reusing 5485's physical-science framings, which suits General Science's breadth and avoids two near-identical files. **Remaining: 02b (Life Science, III-A/III-B) and 02c (Earth & Space, IV-A/IV-B)** — 12 sections of blank-page authoring, no source to draw on. #106 stays open. |
| 2026-09-10 | Pricing settled (D-42); Praxis Core scoped and parked (N-16) | **D-42 — every app free to download with one non-consumable unlock** ($9.99–$14.99, teaching chapters and a diagnostic free, full timed tests and the complete bank paid). The session owner's original free-Mathematics-app model was set aside because its funnel does not exist: the three apps serve **disjoint buyers** (that being D-41's whole basis), so a free app has no second purchase to lead to. It is also the *simpler* build — three apps with one unlock each need one product id and a Restore button, where the original four-product entitlement matrix was the complex version, so D-41's split removed the complexity that made IAP look expensive. Not a subscription: a one-and-done product invites forgotten renewals and one-star reviews. Consequence recorded because it constrains everything downstream — LTV is a single ~$10 net purchase with no repeat business, which rules out paid acquisition and leaves ASO, the test-code keywords, teacher-prep programmes and reviews. **N-16 — Praxis Core scoped rather than assumed.** The claim that Mathematics is "the most common test new teachers need" conflated 5165 (a subject-licensure test for future maths teachers) with **Core Academic Skills** (5713/5723/5733), which nearly every candidate takes to enter a teacher-prep programme — and which is the one product with a genuine funnel *into* the Subject Assessments. All three companions were obtained and read (5713 needed the Preview re-save). **Finding: Core is the most expensive subject family in the catalogue, not the cheapest.** 5581 was the zero-code case; all three Core tests fall outside the settled single-select assumption in different directions — 5733 has multi-select, numeric entry and a *four-function* calculator (5165's is scientific, D-14, and hard-coded to that test, #111); 5723 has two essays this engine cannot score at all; 5713 is built on shared reading passages where the schema has one `stem` per question; and 5713/5723 publish category counts as **ranges**, which `verify.mjs` rejects outright. Parked as a deliberate v2 decision with the STEM app still shipping first; the essay question left open and recorded. Blueprints in BLUEPRINT.md. The transferable lesson: "a subject is data, not code" holds for **Subject Assessments**, the family that guide was written about — not for every Praxis test, and §1d's format check is what caught it before any authoring. |
| 2026-09-10 | session-01 — 5165's answer key redistributed (#93, N-15) | First session of the approved App Store schedule. 198 of 5165's 253 questions were **92.4% keyed "a" with no "d" at all**; now 25.3/25.3/24.5/24.9 across the whole bank, every category between 23.1% and 28.6%. Both of the issue's technical premises were verified rather than assumed and both held: a `json.dumps` round-trip would have grown the file 245KB→322KB and touched every line (so the edit was raw-text), and the file proved perfectly regular — all 1012 option lines and 253 `correct` lines matched one pattern. **Departed from the issue on one point.** Plain SHA-256 rotation fixed the global skew but left **10 of I-A's 21 questions on "d" (48%)** — category being the exact slice #93 used to tabulate the original defect. Rotation is therefore planned per category: within each, target questions are walked in SHA-256 order and assigned the least-used letter. Even *and* unpredictable, which is the combination `i % 4` failed — uniformity was never its problem, its visible cycle was. Adjacent-pair key matches 21.8% against a 25% chance baseline, longest run 4. Verified by asserting per question that the option content set is unchanged, non-target questions are byte-identical, and **the key still resolves to the same option content**; 750 lines changed against 750 added (exactly 150 rotated × 5), zero changed lines outside the two patterns; two questions checked by hand against the mathematics. `ADDING-A-SUBJECT.md` §4 corrected in the same PR: its claim that the skew let a test-taker "score 79% without knowing any mathematics" was **false** — `shuffleQuestionOptions` randomises option order on every assembly path, so the exposure was always the readable public bank file, never the runtime. It now also records that rotating a *shipped* bank silently re-scores every saved attempt, since answers store option ids and correctness is recomputed live — which is why this had to happen before release. |
...
| 2026-09-08 | `ADDING-A-SUBJECT.md` — scope assumptions stated | Session owner narrowed the guide's premise: every future subject will be a standard Praxis Subject Assessment with single-select multiple-choice questions. Recorded as two named assumptions in the guide's opening section, which is also the answer to *why* adding a subject is data work — the runner, scorer and progress store already handle exactly that shape, so a new subject never asks them to do anything new. The previous draft's planning step on question types the runner doesn't implement (numeric entry, multi-select, drag-and-drop, media stimulus) was over-weighted against this and is reduced to a one-line sanity check, deliberately kept rather than deleted: the assumptions are expected to hold, but a future session that silently authored several hundred questions against a companion that contradicted them would be an expensive way to find out. Docs only. |
| 2026-09-08 | `ADDING-A-SUBJECT.md` written — end-to-end procedure for a sixth subject | Session owner asked for the full procedure to be documented for future reference: creating questions and categories, how to structure them, and how to integrate a subject into both this project and the iOS app. Written after auditing what adding a subject actually costs, so the guide reports measured facts rather than intentions. **Headline finding: a subject is data, not code.** Across the whole shipped codebase — 9 JS modules and 5 HTML pages — exactly one file (`run.html`) contains a hardcoded subject code, six occurrences of which two are comments; there is no allow-list of test codes anywhere, and `referencePanel`/`teachingContent` are genuinely optional (proven by 5101 and 5652 shipping without panels). A subject needing no calculator and no reference panel is a **zero-code change**: one bank file, one manifest entry, optional teaching chapters. The document covers blueprint extraction (and where to stop reading the ETS PDFs), the recursive category tree and its permanent-id rule, overlays, the 3×-exam-length authoring target, the question record, the optional reference/teaching layers, the manifest entry, exactly what `tools/verify.mjs` does and does not enforce, the `derivedFrom` path with N-7/N-9's landmines, and the downstream iOS integration under D-19. Two things deliberately given prominence because they were learned the hard way: the **answer-key distribution check** (with a runnable snippet, tested against 5485 before publishing) — 5165's original 198 questions were 79% keyed "a", scoreable without knowing any mathematics — and the reference-panel double-map step, flagged inline as the one part requiring code changes, with an explicit instruction to delete that warning once #111 lands. Revised in the same PR after the session owner clarified the starting point: the trigger is that they drop the new exam's Study Companion PDF into the gitignored `Knowledge-Guides/` folder and point at it, so the guide now opens by stating that and covers what to do first — confirm the PDF actually parses (it refuses encrypted files, which `5165-Mathematics.pdf` is), read only "Test at a Glance"/"Content Topics," check the category counts sum to the stated total, **report the blueprint back and get it confirmed before authoring anything** (roughly 3× formLength questions and a set of permanent category ids all depend on those numbers), confirm the PDF is not showing as untracked in git, and check for question types the runner does not implement — numeric entry, multi-select, drag-and-drop — which would make the subject a code change rather than a data change and needs escalating at the plan gate. Pointers added from CLAUDE.md and README.md. |
...
| 2026-09-08 | Start-card whitespace fix on narrow viewports (N-10) | Reported by the session owner from real use of the iOS wrapper app: on a phone, each S2 Start card had a large dead gap between its description text and its button. Root-caused to axis-relative flex values that were never re-examined when a `@media (max-width: 26rem)` block flips `.start-test-option` from a row to a column — `flex: 1 1 14rem` on `.start-test-text` stopped meaning a 14rem *width* and started meaning a 224px *height*, and the base rule's `justify-content: space-between` then distributed the surplus vertically, stranding the button at the card's bottom. **Measured rather than eyeballed**, against the live GitHub Pages site at a 375px viewport: all five cards exactly 314px tall with text blocks exactly 224px — identical across cards holding visibly different amounts of text, which is what proved the height was imposed by `flex-basis` rather than derived from content. Candidate fix applied in-page and re-measured before touching the stylesheet: cards 151–169px varying with real content, Start section 1570px → 791px, and the 16px text-to-button gap is just the intended `gap: 1rem`. Fix is two declarations inside the existing narrow-viewport block (`justify-content: flex-start`, `.start-test-text { flex: 0 0 auto }`) — no change at any width above 26rem. Logged as **N-10** for the general lesson: any media query that changes `flex-direction` must re-check every inherited `flex-basis`, `justify-content`, and `align-items`, since all are axis-relative and remain syntactically valid while silently changing meaning. No gate caught this — `tools/verify.mjs` validates content, not layout. Also confirmed visually in the iOS app via a temporary preview build (reverted; that repo syncs from `main` per D-30, not from an unmerged branch). |
| 2026-09-03 | "Beat Army!" masthead text linked to the USNA Alumni Association | Ad-hoc request from the session owner: keep the "Go Navy, Beat Army!" easter-egg text on `index.html` exactly as it reads, but make "Beat Army!" a real hyperlink to `http://www.usna.com`. Implemented as an SVG `<a>` wrapping just that `<text>` element, inside the existing hand-authored masthead SVG (D-34). This changes the accessibility posture of half the mark: "Go Navy" stays exactly as it was (purely decorative, `aria-hidden`, not announced), while "Beat Army!" is now a real link and so is left in the accessibility tree — its own visible text is its accessible name, and it is keyboard-focusable like any other link — removed `aria-hidden`/`role="presentation"`/`focusable="false"` from the parent `<svg>` accordingly, since those would have blocked the new link too. No CSS changes needed — the existing `.hub-signature-mark text` rule already styles any `<text>` regardless of an `<a>` wrapper, and the site's global `:focus-visible` outline already covers the new focusable element. Shallow, self-reviewed per Gate 3. Live-verified in the browser: the link's `href`/`tabIndex`/computed `cursor: pointer` all confirmed correct via direct DOM inspection, visual screenshot confirms identical rendering to before, zero console errors. |
| 2026-09-03 | PRs #99/#100 merged; NAS re-synced | Both merges (5101's and 5485's teaching-chapter PRs, the two prior rows) triggered GitHub Pages' own auto-deploy workflow, confirmed live at `https://homesik92.github.io/PRAXIS-Practice/` with zero console errors. The NAS copy does **not** auto-update from a GitHub merge (same gap as the 2026-09-01 row above) and was still serving the pre-merge site — session owner asked directly, re-synced with the same `tar -cf - index.html results.html run.html teach.html test.html css js data \| ssh -i ~/.ssh/praxis_nas_ed25519 -p 24 praxis-deploy@10.0.0.37 'tar -xf - -C /volume1/praxis-practice'` pattern. Verified via SSH read of the NAS's own `manifest.json` (`5101: 360, 5485: 375` — both current) and `ls data/teaching/` (4 files, including the new `5101.json`/`5485.json`), plus a live browser load of `http://10.0.0.37:8080/teach.html?code=5485&category=I-B` showing the correct chapter with zero console errors. Both NAS and GitHub Pages now match `main`. |
