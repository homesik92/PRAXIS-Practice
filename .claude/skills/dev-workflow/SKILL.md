...
- **Grep the staged diff for secrets before every push** (`git diff --staged | grep -iE "password|secret|api_key|token"`) — this repository is public. No credentials, tokens, Apple Developer certificates or provisioning profiles belong in the tree, commit messages, or PR/issue text, ever.
- **No personal information on any publicly viewable surface — and a repo is one.** Real names, emails, phone numbers, addresses, device names, anything that identifies a living person: out of the tree, out of **commit messages**, out of **PR and issue titles and bodies**, out of published docs. Write "the session owner" or "a tester" instead. **Redact on the way out**, while writing the artifact — not afterwards, because rewriting published history costs more than it recovers.

  Three traps. The surfaces most often missed are the ones **outside** the tree, since a commit message and a PR body are published the moment they're pushed and no `.gitignore`, reviewer, or CI step ever looks at them. The usual leak path is **attribution**, where a name arrives quoted from a message or a device label and reads as a citation rather than as personal information — which is exactly why it slips through. And **commit metadata is a surface too**: `user.name` and `user.email` land in public history on every push, so keep the local git identity set to a GitHub `noreply` address.
  ⚠ **No checker can enforce this** — one would have to hold the very list of names it exists to keep out — so it rests on being written here and honored.
...
Some things are the session owner's to run rather than yours: anything that changes repository settings, grants access, touches billing or Apple Developer account credentials and certificates, or touches the NAS itself (its admin interface, shares, web-server configuration, or user accounts). Diagnose, hand over the exact command or click-path, and where possible fix the scripts so the manual step never recurs.
...
| "This API is standard — I know it from memory." | Versions drift — iOS SDKs and Xcode especially. Verify against current official docs or flag it unverified. |
...
**The iOS app is a second production target** (D-44). Production there means a build someone actually studies with — a TestFlight install or an App Store release — not a simulator run. The same two disciplines apply with the same force: saved progress lives in the `WKWebView`'s storage, *outside* the app bundle, and survives every update, so a data-shape change can destroy a real study history exactly as it can on the site; and know the way back — the previous build must be reinstallable through TestFlight before a relied-on install is overwritten. Release steps themselves live in `APP-STORE-ROADMAP.md`.
